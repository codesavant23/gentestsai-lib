...
class PtsuiteSyntacticCorrector:
	...
	def __init__(
		...
		"""
			Creates a new PtsuiteSyntacticCorrector and associates it with the maximum number
			of attempts to be made for each correction series
            
            Parameters
            ----------
                max_tries: int
					An integer indicating the maximum number of attempts to be made in requests
                
                llm_accsor: ILlmApiAccessor
                    An `ILlmApiAccessor` object that will be used to make correction requests
                    
                synt_checker: str
					An `ISyntacticChecker` object representing the syntax checker to be used to
                    test the correctness of the partial test suite after each attempt
                    
                resp_format: str
                    Optional. Default = `None`. A RegEx string containing the format to be used
					to identify the code resulting from the correction attempts made.
                ...
                logger: ATemporalFormattLogger
					...
					to be used to log the stages of each correction attempt (not to log the stages of the request to the LLM)
            
            Raises
            ------
                ValueError
                    Occurs if the `llm_accsor` parameter is `None`
		"""
		if max_tries < 1:
			raise ValueError()
	...
	def has_corr_terminated(self) -> bool:
		"""
			Checks whether the last sequence of correction attempts that was started has ended 
			either because the correction was successful or because the maximum number of attempts was reached—
            
            Returns
            -------
				bool
                    A boolean indicating whether the previously started retry series
                    has ended
                    
            Raises
            ------
                PromptingSessionNeverPerformedError
                    Occurs if a retry series has never been started
		"""
	...
	def has_corr_succ(self) -> bool:
		"""
			Checks whether the last correction attempt made in the completed sequence of
			correction attempts was successful, thereby generating a partial test suite
            ...
            ------
                PromptingSessionNeverPerformedError
                    Occurs if a sequence of correction attempts has never been started
            
                PromptingSessionInProgressError
                    Occurs if a sequence of correction attempts is not currently in progress
		"""
		if not self._corr_everperf:
			raise PromptingSessionNeverPerformedError()
	...
	def get_lastcorr(self) -> str:
		"""
            ...
            To verify the success of the last sequence of correction attempts, you
            must use the `.has_corr_succ(...)` operation
            ...
            -------
                Tuple[str, int]
                    A mixed tuple containing:
                        ...
                        - The code of the last correction attempt for the last partial test suite for which a fix was attempted
                        - The attempt number at which the fix was successful
	...
	def start_new_correction(
		...
		"""
            ...
            ----------
                ptsuite_code: str
					A string containing the code of the partial test suite for which to attempt corrections
                
                resp_timeout: int
                    An integer representing the timeout in milliseconds after which to declare the response failed
					
			Raises:
            ------
                PromptingSessionInProgressError
                    Occurs if a series of correction attempts has already been started
                    but not yet completed
					
				ValueError
                    Occurs if:
                    
                        - The `ptsuite_code` parameter is `None` or an empty string
                        - The `resp_timeout` parameter is less than 1
		"""
	...
	def perform_corr_try(self) -> str:
		"""
            ...
            ------
				...
				WrongResponseFormatError
                    ...
                    with this PtsuiteSyntacticCorrector
                
                SyntacticCheckError
                    Occurs if the syntax checking process cannot be completed. This includes failures
                    caused by the underlying checker, external tool execution, or required source preparation.
		"""
		...
		if not self._try_succ:
			try:
				self._logger.log(
					f"Starting syntactic correction request (Attempt {self._times_tried}/{self._max_tries}) ..."
	...
	def _is_syntact_correct(self) -> bool:
		"""
			Perform a syntactic check on the latest partial test suite generated
